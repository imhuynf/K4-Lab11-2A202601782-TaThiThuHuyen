...
from agents.agent import chat_with_agent, create_unsafe_agent, create_protected_agent
...
async def run_comparison():
    ...
    # --- Protected agent: replay the exact same corpus for a fair comparison. ---
...
def print_comparison(unprotected, protected):
    ...
    for i, (u, p) in enumerate(zip(unprotected, protected), 1):
        u_status = (
            "LEAKED" if u.get("leaked") else "BLOCKED" if u.get("blocked") else "PASSED"
        )
        p_status = (
            "LEAKED" if p.get("leaked") else "BLOCKED" if p.get("blocked") else "PASSED"
        )
